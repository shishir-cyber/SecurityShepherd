...
/**
 * The add players server side functionality is hanled by this control class <br>
 * <br>
...
public class AddPlayer extends HttpServlet {
  ...
  /**
   * Initiated by addPlayers.jsp. Player is ether added to the system under a specific or null class
   * or invalid data is detected and no player is added to the database Adding of player to Core
   * Database is handed by Setter.createUser
   *
      ...
      if (Validate.validateTokens(tokenCookie, tokenParmeter)) {
        ...
        try {
          ...
          if (basicValidation && userValidate && classInfo != null) {
            ...
            if (Setter.userCreate(
                ApplicationRoot, classId, userName, passWord, "player", userAddress, true)) {
              if (userAddress.isEmpty()) {
              ...
              reponseMessage =
                  "<p>"
                      ...
                      + "</a> added to <a>"
                      + Encode.forHtml(classInfo[0] + " " + classInfo[1])
              ...
              reponseMessage =
                  "<div id='error' class='informationBox'><p colour='red'><strong>"
                      + Encode.forHtml(userName)
                      + " was not added to the system due to an error."
                      + "</strong></p></div>";
            }
